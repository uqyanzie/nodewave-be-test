...
import { SalesDTO } from "$entities/Sales";


export async function getSales(filter: FilteringQueryV2) : Promise<ServiceResponse<PaginationResponse<SalesDTO>>> {
...
export async function uploadSales(body: { file?: Express.Multer.File, fileUrl?: string, fileName?: string}) : Promise<ServiceResponse<string>>{
    const {file, fileUrl, fileName} = body
    
    const taskIdentifier = 'uploadSales'
...
export async function getSalesFiles (filter: FilteringQueryV2) : Promise<ServiceResponse<PaginationResponse<FileUploadDAO>>> {
    const filterQuery = buildFilterQueryLimitOffsetV2(filter)
